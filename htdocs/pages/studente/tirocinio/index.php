					...
					<div class="media">
						...
						<div class="media-content">
							<p><strong>Inizio:&#9;</strong>
								<time datetime="<?= $t_ini ?>"><?= $t_ini ?></time>
							</p>
				...
				<?php if ($status == 1) { ?>
					<div class="control" data-tab="editor" hidden>
						<textarea id="resoconto" class="textarea" rows="30"
								  title="resoconto" <?php if ($t_vis == "azienda") echo 'readonly'; ?> ><?= filter_var($t_desc, FILTER_SANITIZE_FULL_SPECIAL_CHARS) ?></textarea>
					</div>
				<?php }
				if ($status !== 0)
				{ ?>
					<div data-tab="preview" hidden>
								...
								<div class="media">
									...
									<p class="media-right">
										...
										<a class="button is-success"
										   id="bt_publish" <?= $t_vis !== "studente" ? "disabled" : "href=\"publish.php?tirocinio={$_GET['tirocinio']}\"" ?>>
											<span class="icon">
						...
						<?php } ?>
						<p class="content"
						   id="preview_editor"><?php if ($t_vis == 'azienda') echo sanitize_html($t_desc); ?></p>
					</div>
				<?php } ?>
